...
yargs(hideBin(process.argv))
    ...
    description: "Port to run the local development server on. Overrides the frontend port set in the .env file.",
    type: "number",
    alias: "p",
})
    .option("preview", {
    ...
    .command("$0 [example]", "Starts a local development for the app in /src, or an example app if specified", (yargs) => {
    ...
    if (args.example && !Context.examples.includes(args.example)) {
        const matches = Context.examples.filter((ex) => ex.endsWith("/" + args.example) ||
    ...
    .command("examples", "Displays a list of available examples to choose from", () => { }, async (args) => {
    const { example } = await prompts({
        ...
        suggest: async (input, choices) => choices.filter((choice) => choice.title.toLowerCase().includes(input.toLowerCase())),
    }, {
        onCancel: () => {
            console.log(errorChalk("Aborted by the user."));
            process.exit(0);
