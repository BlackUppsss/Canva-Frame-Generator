...
export class AppRunner {
    async run(ctx: Context) {
        console.log(infoChalk("Info:"), `Starting development server for ${highlightChalk(ctx.entryDir)}\n`);
        if (!ctx.hmrEnabled) {
            console.log(`${infoChalk("Note:")} Hot Module Replacement (HMR) not enabled. To enable it, please refer to the instructions in the ${highlightChalk("README.md")}\n`);
        }
    ...
    private readonly maybeRunBackendServer = async (ctx: Context, table: Table.Table, cert: Certificate | undefined, webpackDevServer: WebpackDevServer) => {
        if (!ctx.developerBackendEntryPath) {
            return;
        }
        if (!ctx.appId) {
            console.log(errorChalk("Error:"), `'CANVA_APP_ID' environment variable is undefined. Refer to the instructions in the ${highlightChalk("README.md")} on starting a backend example.`);
        ...
        if (ctx.ngrokEnabled) {
            console.log(warnChalk("Warning:"), `ngrok exposes a local port via a public URL. Be mindful of what's exposed and shut down the server when it's not in use.\n`);
        }
    ...
    private readonly runWebpackDevServer = async (ctx: Context, table: Table.Table, cert: Certificate | undefined): Promise<WebpackDevServer> => {
