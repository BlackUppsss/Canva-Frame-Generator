import { Box, Button, FormField, Grid, ImageCard, NumberInput, Rows, Text, } from "@canva/app-ui-kit";
import { type AppElementOptions, initAppElement, type ImageRef, } from "@canva/design";
...
export const App = () => {
    ...
    const { data: { imageId, width, height, rotation }, } = state;
    ...
    const addOrUpdateImage = useCallback(async (operation: Operation) => {
        setLoading(true);
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        ...
        <Button variant="primary" onClick={() => addOrUpdateImage(Operation.Add)} disabled={disabled} stretch>
          Add image
        </Button>
        {state.update && (<Button variant="primary" onClick={() => addOrUpdateImage(Operation.Update)} disabled={disabled} stretch>
            Update image
          </Button>)}
      </Rows>
    </div>);
