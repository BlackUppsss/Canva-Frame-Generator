import { Button, LoadingIndicator, Rows, Title, Text, Box, MultilineInput, FormField, Columns, Column, } from "@canva/app-ui-kit";
...
export function App() {
    const metaOauth = useMemo(() => auth.initOauth({ type: "single_account", provider: META_PROVIDER }), []);
    const googleOauth = useMemo(() => auth.initOauth({
        type: "single_account",
    ...
    const setStatus = (provider: typeof META_PROVIDER | typeof GOOGLE_PROVIDER, updates: Partial<ProviderStatus>) => {
    ...
    const authorize = useCallback(async (provider: typeof META_PROVIDER | typeof GOOGLE_PROVIDER) => {
    ...
    const retrieveAndSetToken = useCallback(async (provider: typeof META_PROVIDER | typeof GOOGLE_PROVIDER, forceRefresh = false) => {
        const oauth = getOauthClient(provider);
        const scope = getScope(provider);
    ...
    const logout = useCallback(async (provider: typeof META_PROVIDER | typeof GOOGLE_PROVIDER) => {
        const oauth = getOauthClient(provider);
        setStatus(provider, {
            accessTokenResponse: undefined,
            loading: true,
    ...
    const fetchData = useCallback(async (provider: typeof META_PROVIDER | typeof GOOGLE_PROVIDER) => {
        const status = getStatus(provider);
        const accessToken = status.accessTokenResponse?.token;
        if (!accessToken) {
            return;
        }
        const backendUrl = provider === META_PROVIDER ? META_BACKEND_URL : GOOGLE_BACKEND_URL;
        setStatus(provider, { loading: true });
        try {
    ...
    const renderProviderSection = (provider: typeof META_PROVIDER | typeof GOOGLE_PROVIDER, providerDisplayName: string) => {
        const status = getStatus(provider);
        const loading = status.loading && status.accessTokenResponse === undefined;
        const isConnected = status.accessTokenResponse != null;
        return (<Box padding="3u" border="standard" borderRadius="standard" background="neutral">
        <Rows spacing="2u">
          <Title size="small">{providerDisplayName} Provider</Title>

          {status.error ? (<Rows spacing="1u">
              <Text tone="critical">Error: {status.error}</Text>
              <Button variant="primary" onClick={() => authorize(provider)} disabled={loading}>
                Try again
              </Button>
            </Rows>) : loading ? (<LoadingIndicator />) : !isConnected ? (<Rows spacing="2u">
              <Text>
            ...
            </Rows>) : (<Rows spacing="2u">
              <Text>Connected to {providerDisplayName}!</Text>
              <Columns spacing="1u">
                <Column>
                  <Button variant="secondary" onClick={() => logout(provider)} disabled={loading}>
                    Log out
                  </Button>
                </Column>
                <Column>
                  <Button variant="primary" onClick={() => fetchData(provider)} disabled={loading}>
                    Fetch data
                  </Button>
                </Column>
              </Columns>
              {status.responseBody ? (<FormField label="Response" value={JSON.stringify(status.responseBody, null, 2)} control={(props) => (<MultilineInput {...props} maxRows={5} autoGrow readOnly/>)}/>) : null}
            </Rows>)}
        </Rows>
      </Box>);
    };
    const result = (<div className={styles.scrollContainer}>
      <Box justifyContent="center" width="full" alignItems="center" display="flex" height="full">
        <Rows spacing="3u">
          <Rows spacing="2u">
