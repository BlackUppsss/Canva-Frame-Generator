import { Button, FormField, MultilineInput, Rows, Text, TextInput, } from "@canva/app-ui-kit";
...
export const App = () => {
    ...
    const getDesignData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${BACKEND_HOST}/design?designToken=${designToken.token}`, {
                headers: {
    ...
    const saveDesignData = async () => {
        ...
        setError(undefined);
        try {
            const [designToken, authToken, { defaultPageDimensions: dimensions }] = await Promise.all([
                getDesignToken(),
                auth.getCanvaUserToken(),
                getDesignMetadata(),
            ]);
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="3u">
        ...
        <MultilineInput autoGrow readOnly value={JSON.stringify(designData, null, 2)}/>
        <Button loading={state === "loading"} disabled={state === "loading"} variant="primary" onClick={refreshDesignData}>
          Refresh
        </Button>
        <FormField control={(props) => <TextInput {...props} onChange={setTitle}/>} label="Title"/>
        <Button loading={state === "loading"} disabled={state === "loading"} variant="primary" onClick={saveDesignData}>
          Save
        </Button>
