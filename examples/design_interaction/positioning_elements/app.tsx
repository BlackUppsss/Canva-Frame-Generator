import { Alert, Box, Button, FormField, Grid, ImageCard, Rows, Select, Text, } from "@canva/app-ui-kit";
...
export const App = () => {
    ...
    const isRequiredFeatureSupported = isSupported(addElementAtPoint, getCurrentPageContext);
    const [state, setState] = useState<UIState>(initialState);
    const { data: { imageId }, } = state;
    const disabled = !imageId || imageId.trim().length < 1;
    const getPlacement = async (placement?: ElementPlacement): Promise<Placement | undefined> => {
        const pageContext = await getCurrentPageContext();
        const pageDimensions = pageContext.dimensions;
        if (!pageDimensions) {
            return;
        }
        const elementSize = Math.min(pageDimensions.height, pageDimensions.width) / 2;
        switch (placement) {
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        ...
        <FormField label="Select an image" control={({ id }) => (<Box id={id} padding="1u">
              <Grid columns={3} spacing="1.5u">
                {items.map((item) => (<ImageCard ariaLabel="Add image to design" alt={item.title} key={item.key} thumbnailUrl={item.imageSrc} onClick={item.onClick} selectable={true} selected={item.active} borderRadius="standard"/>))}
              </Grid>
            </Box>)}/>
        <FormField label="Placement" value={state.placement} control={(props) => (<Select {...props} options={[
        ...
        <Button variant="secondary" onClick={addOrUpdateAppImage} disabled={disabled || !isRequiredFeatureSupported}>
          {state.update ? "Update app element" : "Add app element"}
        </Button>
        <Button variant="secondary" onClick={addImage} disabled={disabled || !isRequiredFeatureSupported}>
          Add element
        </Button>
