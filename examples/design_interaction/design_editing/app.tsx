...
export const App = () => {
    ...
    function checkAbsolute(page: DesignEditing.Page): asserts page is DesignEditing.AbsolutePage {
    ...
    const replaceAllFillsWithDogs = () => {
        openDesign({ type: "current_page" }, async (session) => {
            ...
            const elementsToReplace = session.page.elements.filter((el) => el.type === "rect" || el.type === "shape");
            ...
            elementsToReplace.forEach((element) => {
                switch (element.type) {
                    case "rect":
                        ...
                        break;
                    case "shape":
                        element.paths.forEach((p) => p.fill.isMediaEditable && p.fill.mediaContainer.set(dogMedia));
                        break;
                    default:
    ...
    const addTextWithSparkles = () => {
        ...
        openDesign({ type: "current_page" }, async (session) => {
            ...
            const insertedText = session.page.elements.insertBefore(undefined, elementStateBuilder.createTextElement({
            ...
            session.page.elements.insertBefore(insertedText, elementStateBuilder.createRectElement({
            ...
            session.page.elements.insertAfter(insertedText, elementStateBuilder.createRectElement({
    ...
    async function groupAllSupportedElements() {
        return openDesign({ type: "current_page" }, async (session) => {
            ...
            const elsToGroup = session.page.elements.filter((el) => el.type === "embed" ||
                el.type === "rect" ||
                el.type === "shape" ||
                el.type === "text");
            if (elsToGroup.length < 2) {
                setError("Need at least 2 supported elements (embed, rect, shape or text) to group them");
                return;
            }
    ...
    async function insertAndGroup() {
        return openDesign({ type: "current_page" }, async (session) => {
            ...
            const shape = session.page.elements.insertBefore(undefined, elementStateBuilder.createShapeElement({
            ...
            textRange.formatParagraph({ index: 0, length: 10 }, { fontSize: 45, color: "#000000", textAlign: "center" });
            const text = session.page.elements.insertAfter(shape, elementStateBuilder.createTextElement({
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        ...
        <Button variant="secondary" onClick={flipDesign} disabled={operation !== Operation.NONE} loading={operation === Operation.FLIP}>
          Flip design
        </Button>
        <Button variant="secondary" onClick={deleteAllTextElements} disabled={operation !== Operation.NONE} loading={operation === Operation.DELETE}>
          Delete all text elements
        </Button>
        <Button variant="secondary" onClick={replaceAllFillsWithDogs} disabled={operation !== Operation.NONE} loading={operation === Operation.UPDATE}>
          Replace all fills with dogs
        </Button>
        <Button variant="secondary" onClick={addTextWithSparkles} disabled={operation !== Operation.NONE} loading={operation === Operation.INSERT}>
          Add some sparkly text
        </Button>
        <Button variant="secondary" onClick={groupAllSupportedElements} disabled={operation !== Operation.NONE} loading={operation === Operation.GROUP}>
          Group elements on page
        </Button>
        <Button variant="secondary" onClick={insertAndGroup} disabled={operation !== Operation.NONE} loading={operation === Operation.INSERT_AND_GROUP}>
