...
export const App = () => {
    ...
    const updateFormatting = async () => {
        ...
        for (const richtext of draft.contents) {
            ...
            regions.forEach((region) => {
                richtext.formatText({ index, length: region.text.length }, {
                    fontWeight: region.formatting?.fontWeight === "bold" ? "normal" : "bold",
                });
    ...
    const replaceText = async () => {
        ...
        for (const richtext of draft.contents) {
            ...
            richtext.replaceText({ index: 0, length: plaintext.length }, "Hello World", { decoration: "underline" });
        }
        await draft.save();
    };
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        ...
        <Button variant="primary" onClick={updateFormatting} disabled={selectionState.count === 0}>
          Toggle bold
        </Button>
        <Button variant="primary" onClick={appendText} disabled={selectionState.count === 0}>
          Append "!"
        </Button>
        <Button variant="primary" onClick={replaceText} disabled={selectionState.count === 0}>
