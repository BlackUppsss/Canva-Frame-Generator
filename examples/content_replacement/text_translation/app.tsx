...
export const App = () => {
    ...
    const translateWithFormatting = async () => {
        ...
        await editContent({
            ...
            const request = session.contents.map((range) => range.readTextRegions().map((region) => region.text));
            const response = await getTranslation(request);
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        ...
        <Button variant="secondary" onClick={translateWithFormatting} disabled={inProgressTask != null} loading={inProgressTask === Task.WITH_FORMATTING}>
          Translate with formatting
        </Button>
        <Button variant="secondary" onClick={translateWithoutFormatting} disabled={inProgressTask != null} loading={inProgressTask === Task.WITHOUT_FORMATTING}>
