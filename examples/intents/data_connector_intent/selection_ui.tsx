...
import { Alert, Button, Rows, Text, CheckboxGroup, FormField, Title, } from "@canva/app-ui-kit";
...
export const SelectionUI = (request: RenderSelectionUiRequest) => {
        ...
        switch (reason) {
            case "data_selection":
                if (request.invocationContext.dataSourceRef) {
                    try {
                        const savedParams = JSON.parse(request.invocationContext.dataSourceRef.source) as RealEstateDataConfig;
            ...
            case "outdated_source_ref":
                setError("Your previously selected data is no longer available. Please make a new selection.");
                break;
            case "app_error":
                setError(request.invocationContext.message ||
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (result.status === "completed") {
                ...
                setError(result.status === "app_error" && "message" in result
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        ...
        {success && (<Alert tone="positive" title="Data preview loaded successfully!"/>)}
        <Text variant="bold">
          Sydney construction project sales in each release stage
        </Text>
        <Rows spacing="1u">
          <FormField label="Release Stage" control={(props) => (<CheckboxGroup {...props} value={selectedStageFilter} options={saleStageOptions.map((stage) => {
                return {
                    label: stage,
                    value: stage,
                };
