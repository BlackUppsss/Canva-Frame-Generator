import type { Preview, PreviewMedia, RenderPreviewUiRequest, } from "@canva/intents/content";
import { useEffect, useState } from "react";
import { useIntl } from "react-intl";
import { isImagePreviewReady, parsePublishSettings } from "./types";
import { Box, Text, Rows, ImageCard, Placeholder, TextPlaceholder, } from "@canva/app-ui-kit";
type PreviewData = Parameters<Parameters<RenderPreviewUiRequest["registerOnPreviewChange"]>[0]>[0];
export const PreviewUi = ({ registerOnPreviewChange, }: RenderPreviewUiRequest) => {
    ...
    return (<Box display="flex" alignItems="center" justifyContent="center" flexDirection="column" width="full" height="full">
      {outputType?.id === "listing" && (<ListingPreview previewMedia={previewMedia} settings={publishSettings}/>)}
    </Box>);
};
const ListingPreview = ({ previewMedia, settings, }: {
    previewMedia: PreviewMedia[] | undefined;
    settings: ReturnType<typeof parsePublishSettings>;
}) => {
    const intl = useIntl();
    const isLoading = !previewMedia;
    return (<Box background="surface" borderRadius="large" padding="2u" border="standard">
      <Rows spacing="2u">
...
const Caption = ({ isLoading, caption, }: {
    ...
    return (<Text lineClamp={3} size="small">
      {caption}
    </Text>);
};
const ImagePreview = ({ previewMedia, }: {
    ...
    return (<Box borderRadius="large">
      {preview ? (<PreviewRenderer preview={preview}/>) : (<Placeholder shape="rectangle"/>)}
    </Box>);
};
const PreviewRenderer = ({ preview }: {
    ...
    return (<Box padding="2u" display="flex" alignItems="center" justifyContent="center">
