import { Button, Column, Columns, GridViewIcon, ListBulletLtrIcon, Rows, SearchInputMenu, Select, } from "@canva/app-ui-kit";
...
export const ListingSearchFilters = ({ query, onQueryChange, layout, onLayoutToggle, propertyType, onPropertyTypeChange, sort, onSortChange, }: ListingSearchFiltersProps) => {
    const intl = useIntl();
    const [queryValue, setQueryValue] = useState(query);
    return (<Rows spacing="2u">
      <SearchInputMenu value={queryValue} onChange={setQueryValue} onChangeComplete={() => onQueryChange(queryValue)} onClear={() => {
            setQueryValue("");
            onQueryChange("");
      ...
      <Columns spacing="1u" alignY="center">
        <Column width="content">
          <Button icon={layout === "grid" ? GridViewIcon : ListBulletLtrIcon} type="button" onClick={onLayoutToggle} variant="secondary" ariaLabel={intl.formatMessage({
            defaultMessage: "Toggle layout between grid and list. Current layout is {currentLayout}",
        ...
        <Column width="1/3">
          ...
          <Select value={propertyType} onChange={onPropertyTypeChange} options={[
            {
