import { Avatar, Box, Button, FormField, Rows, Select, Text, } from "@canva/app-ui-kit";
...
export const OfficeSelectionPage = () => {
    ...
    const intl = useIntl();
    const { handleSubmit, setValue, watch, formState: { touchedFields, isSubmitted }, } = useForm<FormValues>({
        mode: "onSubmit",
        defaultValues: {
            office: "",
        },
      ...
      <Box height="full" display="flex" flexDirection="column" justifyContent="spaceBetween">
        <form onSubmit={handleSubmit(onSubmit)}>
          <Rows spacing="2u">
            ...
            <Rows spacing="0.5u">
              <Button variant="primary" stretch type="submit" disabled={isSubmitted && !selectedOffice}>
                {intl.formatMessage({
            defaultMessage: "Continue",
            description: "Button text to proceed with selection",
        })}
              </Button>
              {isSubmitted && !selectedOffice && (<Text variant="regular" tone="critical">
                  {intl.formatMessage({
                defaultMessage: "Please select an office and agent",
                description: "Text to prompt user to select office and agent",
            })}
                </Text>)}
            </Rows>
          </Rows>
          ...
          <Rows spacing="1u">
            ...
            <Rows spacing="1u" align="start">
              <Box display="flex" alignItems="start">
                <Box paddingEnd="1u">
                  <Avatar photo={mockUserData.userAvatarUrl} name={mockUserData.userName || ""}/>
                </Box>
                <Rows spacing="0">
