import { Box, Column, Columns, Placeholder, Rows, Text, Title, tokens, } from "@canva/app-ui-kit";
...
export const LoadingPage = () => {
    ...
    const office = (useLocation().state as {
        office: Office;
    })?.office;
    const intl = useIntl();
    ...
    if (isLoading) {
        return (<Box height="full" paddingY="2u" display="flex" flexDirection="column">
        <Breadcrumb />
    ...
    if (isError) {
        return (<Box height="full" paddingY="2u" display="flex" flexDirection="column">
        <Breadcrumb />
        <Box height="full" width="full" paddingX="2u" justifyContent="center" alignItems="center" display="flex" flexDirection="column">
    ...
    if (isEmpty) {
        return (<Box height="full" paddingY="2u" display="flex" flexDirection="column">
        <Breadcrumb />
        <Box height="full" width="full" justifyContent="center" alignItems="center" display="flex" flexDirection="column" paddingX="2u">
          <Rows spacing="2u" align="center">
            <Title alignment="center">
              {intl.formatMessage({
                defaultMessage: "Nothing here yet",
                description: "Title shown when there are no agents or listings to display",
            })}
            </Title>
            <Text alignment="center">
              {intl.formatMessage({
                defaultMessage: "There are no agents or listings here. Try choosing another office.",
                description: "Helper text shown when there are no agents or listings to display",
            })}
            </Text>
          </Rows>
        </Box>
