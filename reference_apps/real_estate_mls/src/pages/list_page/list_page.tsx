import { Box, Tab, TabList, TabPanel, TabPanels, Tabs, } from "@canva/app-ui-kit";
...
export const ListPage = () => {
    ...
    const intl = useIntl();
    return (<Tabs onSelect={(value) => navigate(`/list/${value}`, { state: { office } })} activeId={tab} height="fill">
      <Box height="full" display="flex" flexDirection="column" paddingTop="2u">
        <Breadcrumb />
        <Box paddingBottom="1u">
          <TabList>
            <Tab id="listings" active={tab === "listings"} onClick={() => navigate(`/list/listings`, { state: { office } })}>
              {intl.formatMessage({
            defaultMessage: "Listings",
            description: "Tab label for property listings",
        })}
            </Tab>
            <Tab id="agents" active={tab === "agents"} onClick={() => navigate(`/list/agents`, { state: { office } })}>
              {intl.formatMessage({
