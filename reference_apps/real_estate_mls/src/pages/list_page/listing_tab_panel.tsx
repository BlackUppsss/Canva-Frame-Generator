...
import { GridPlaceholder, ListPlaceholder, } from "../../components/placeholders/placeholders";
...
export const ListingTabPanel = ({ office }: ListingTabContentProps) => {
    ...
    const { data: listingItems, hasNextPage, isLoading, fetchNextPage, isFetchingNextPage, isError, } = useInfiniteQuery({
    ...
    return (<Scrollable>
      <InfiniteScroll loadMore={() => fetchNextPage()} hasMore={hasNextPage} useWindow={false}>
        <Box height="full">
          <Rows spacing="2u">
            {((!isError && !isLoading && !!listings.length) ||
            !!query?.length) && (<ListingSearchFilters query={query} onQueryChange={setQuery} layout={layout} onLayoutToggle={toggleLayout} propertyType={propertyType} onPropertyTypeChange={setPropertyType} sort={sort} onSortChange={setSort}/>)}
            {isError && (<Text tone="critical">
                {intl.formatMessage({
                defaultMessage: "Error loading listings. Please try again.",
                description: "Error message when listings fail to load",
            })}
              </Text>)}
            {isLoading && (<Box width="full" paddingBottom="2u">
                {layout === "grid" ? <GridPlaceholder /> : <ListPlaceholder />}
              </Box>)}
            {!isError && !isLoading && !listings.length && (<Box height="full" display="flex" alignItems="center" justifyContent="center">
                <Text>
                  {intl.formatMessage({
            ...
            {!isError && !isLoading && !!listings.length && (<Rows spacing={layout === "grid" ? "2u" : "0"}>
                {layout === "grid" ? (<ListingGrid listings={listings} onListingClick={handleListingClick}/>) : (<ListingList listings={listings} onListingClick={handleListingClick}/>)}
                {isFetchingNextPage && (<Box width="full" paddingBottom="2u">
                    {layout === "grid" ? (<GridPlaceholder />) : (<ListPlaceholder />)}
                  </Box>)}
              </Rows>)}
          </Rows>
