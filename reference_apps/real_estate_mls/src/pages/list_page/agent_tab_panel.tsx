...
export const AgentTabPanel = ({ office }: AgentTabContentProps) => {
    ...
    const { data: agentItems, hasNextPage, isLoading, fetchNextPage, isFetchingNextPage, isError, } = useInfiniteQuery({
      ...
      <InfiniteScroll loadMore={() => fetchNextPage()} hasMore={hasNextPage} useWindow={false}>
        <Box height="full">
          <Rows spacing="2u">
            ...
            query?.length) && (<AgentSearchFilters query={query} onQueryChange={setQuery} layout={layout} onLayoutToggle={toggleLayout} sort={sort} onSortChange={setSort}/>)}
            ...
            {!isError && !isLoading && agents.length === 0 && (<Box height="full" display="flex" alignItems="center" justifyContent="center">
                <Text>
                  {intl.formatMessage({
            ...
            {!isError && !isLoading && agents.length > 0 && (<Rows spacing={layout === "grid" ? "2u" : "0"}>
                {layout === "grid" ? (<AgentGrid agents={agents} onAgentClick={handleAgentClick}/>) : (<AgentList agents={agents} onAgentClick={handleAgentClick}/>)}
