...
export function buildConfig({ devConfig, appEntry = path.join(process.cwd(), "src", "index.tsx"), backendHost = process.env.CANVA_BACKEND_HOST, inHarness = process.env.IN_HARNESS?.toLowerCase() === "true", }: {
    devConfig?: DevConfig;
    appEntry?: string;
    backendHost?: string;
    inHarness?: boolean;
    ...
    if (!backendHost) {
        console.warn(chalk.yellow.bold("BACKEND_HOST is undefined."), `If your app requires a backend, refer to "Customizing the backend host" in the README.md for more information.`);
    }
...
function buildDevConfig(options?: DevConfig): {
    ...
    if (enableHmr && appOrigin) {
        ...
        if (enableHmr && !appOrigin) {
            console.warn("Attempted to enable Hot Module Replacement (HMR) without configuring App Origin... Disabling HMR.");
        }
