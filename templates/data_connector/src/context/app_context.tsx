...
export interface AppContextType {
    ...
    setDataSourceHandler: (value: DataSourceHandler<DataSourceConfig, APIResponseItem>) => void;
...
export const ContextProvider = ({ renderSelectionUiRequest, children, }: {
    renderSelectionUiRequest: RenderSelectionUiRequest;
    children: ReactNode;
}): JSX.Element => {
    const [appError, setAppError] = useState<string>("");
    const [request] = useState<RenderSelectionUiRequest>(renderSelectionUiRequest);
    const [accessToken, setAccessToken] = useState<AccessTokenResponse | undefined>(undefined);
    const oauth = auth.initOauth();
    const isAuthenticated = !!accessToken;
    const [dataSourceHandler, setDataSourceHandler] = useState<DataSourceHandler<DataSourceConfig, APIResponseItem>>();
    const [dataSourceConfig, setDataSourceConfig] = useState<DataSourceConfig>();
    const loadDataSource = useCallback(async (title: string, source: DataSourceConfig) => {
        ...
        if (result.status === "remote_request_failed") {
            ...
            setAppError(`Failed to load data source: ${result.message || result.status}`);
        }
        else {
            setAppError("");
        }
