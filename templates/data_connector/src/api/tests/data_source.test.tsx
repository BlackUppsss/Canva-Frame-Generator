...
describe("DataSourceHandler", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        handler = new DataSourceHandler<TestConfig, TestResponse>(mockConfig, mockColumns, mockFetchData, mockSelectionPage, mockConfigPage);
    });
    ...
    test("fetchAndBuildTable calls fetchData with correct parameters", async () => {
        ...
        expect(mockFetchData).toHaveBeenCalledWith(mockConfig, authToken, rowLimit, signal);
    ...
    test("fetchAndBuildTable throws DataAPIError when fetchData fails", async () => {
        ...
        await expect(handler.fetchAndBuildTable(mockConfig, "token", 10, undefined)).rejects.toThrow(DataAPIError);
    });
});
