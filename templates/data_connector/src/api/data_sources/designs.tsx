...
import { ownershipFilter, sortOrderField, } from "src/components/inputs/messages";
...
export const designsSource = new DataSourceHandler<DesignsDataSource, CanvaDesign>({
    schema: "designs/v1",
    query: "",
    ownership: "any",
    sort_by: "relevance",
...
], (source: DesignsDataSource, authToken: string, rowLimit: number, signal: AbortSignal | undefined) => getDesigns(authToken, rowLimit, signal, source.query, source.ownership, source.sort_by), DesignSelection, DesignsSourceConfig);
export async function getDesigns(authToken: string, rowLimit: number, signal: AbortSignal | undefined, query: string, ownership: string, sort_by: string, continuation?: string, allItems: CanvaDesign[] = []): Promise<CanvaDesign[]> {
    ...
    return fetch(url, {
        ...
        if (data.continuation && updatedItems.length < rowLimit) {
            return getDesigns(authToken, rowLimit, signal, query, ownership, sort_by, data.continuation, updatedItems);
        }
        return updatedItems;
    });
}
function DesignSelection() {
    ...
    const title = intl.formatMessage({
        ...
        description: "Main heading on the designs button displayed when selecting the import type.",
    });
    const description = intl.formatMessage({
        defaultMessage: "Query designs",
        description: "Subtext on the designs selection button displayed when selecting the import type.",
    });
    const handleClick = () => {
        setDataSourceHandler(designsSource as unknown as DataSourceHandler<DataSourceConfig, APIResponseItem>);
        navigate(Paths.DATA_SOURCE_CONFIG);
    };
    return (<HorizontalCard key="designs" title={title} thumbnail={{ icon: () => <DatabaseIcon /> }} onClick={handleClick} description={description} ariaLabel={description}/>);
}
function DesignsSourceConfig(sourceConfig: DesignsDataSource) {
    ...
    useEffect(() => {
        setFilterCount((ownership !== "any" ? 1 : 0) + (sortOrder !== "relevance" ? 1 : 0));
    }, [ownership, sortOrder]);
    const resetFilters = () => {
        setOwnership("any");
