...
import { Button, HorizontalCard, Rows, TableMergedHeaderCellsIcon, } from "@canva/app-ui-kit";
import { useEffect, useState } from "react";
import { useIntl } from "react-intl";
import { useNavigate } from "react-router-dom";
import { Header } from "src/components/header";
import { datasetFilter, ownershipFilter, sortOrderField, } from "src/components/inputs/messages";
...
import { dateCell, stringCell } from "src/utils/data_table";
...
export const brandTemplatesSource = new DataSourceHandler<BrandTemplatesDataSource, CanvaBrandTemplate>({
...
], (source: BrandTemplatesDataSource, authToken: string, rowLimit: number, signal: AbortSignal | undefined) => getBrandTemplates(authToken, rowLimit, signal, source.query, source.ownership, source.dataset, source.sort_by), BrandTemplatesSelection, BrandTemplatesSourceConfig);
export async function getBrandTemplates(authToken: string, rowLimit: number, signal: AbortSignal | undefined, query: string, ownership: string, dataset: string, sort_by: string, continuation?: string, allItems: CanvaBrandTemplate[] = []): Promise<CanvaBrandTemplate[]> {
    ...
    return fetch(url, {
        ...
        .then((response) => {
        if (!response.ok) {
            throw new DataAPIError(`Canva Connect response was not ok: ${response.statusText || response.status}`);
        }
        ...
        if (data.continuation && updatedItems.length < rowLimit) {
            return getBrandTemplates(authToken, rowLimit, signal, query, ownership, dataset, sort_by, data.continuation, updatedItems);
        }
        return updatedItems;
    });
}
function BrandTemplatesSelection() {
    ...
    const title = intl.formatMessage({
        ...
        description: "Main heading on the brand templates button displayed when selecting the import type.",
    });
    const description = intl.formatMessage({
        defaultMessage: "Query brand templates",
        description: "Subtext on the brand templates button displayed when selecting the import type.",
    });
    const handleClick = () => {
        setDataSourceHandler(brandTemplatesSource as unknown as DataSourceHandler<DataSourceConfig, APIResponseItem>);
        navigate(Paths.DATA_SOURCE_CONFIG);
    };
    return (<HorizontalCard key="brand-templates" title={title} thumbnail={{ icon: () => <TableMergedHeaderCellsIcon /> }} onClick={handleClick} description={description} ariaLabel={description}/>);
}
function BrandTemplatesSourceConfig(sourceConfig: BrandTemplatesDataSource) {
    ...
    useEffect(() => {
        setFilterCount((ownership !== "any" ? 1 : 0) +
            (dataset !== "any" ? 1 : 0) +
    ...
    return (<div>
      ...
      <Rows spacing="2u">
        <SearchFilter value={query} onChange={setQuery} filterCount={filterCount} resetFilters={resetFilters}>
          <SelectField label={intl.formatMessage(ownershipFilter.label)} options={ownershipOptions} value={ownership} onChange={setOwnership}/>
          <SelectField label={intl.formatMessage(datasetFilter.label)} options={datasetOptions} value={dataset} onChange={setDataset}/>
          <SelectField label={intl.formatMessage(sortOrderField.label)} options={sortOrderOptions} value={sortOrder} onChange={setSortOrder}/>
        </SearchFilter>

        <Button variant="primary" loading={isLoading} onClick={async () => {
            loadTemplates();
        }}>
          {intl.formatMessage({
