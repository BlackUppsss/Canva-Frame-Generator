...
import type { PublishRefValidityState, PublishSettingsSettingsUiContext, RenderSettingsUiRequest, } from "@canva/intents/content";
import { useCallback, useEffect, useState } from "react";
import { useIntl } from "react-intl";
import * as styles from "styles/components.css";
import type { PublishSettings } from "./types";
import { parsePublishSettings } from "./types";
export const SettingsUi = ({ invocationContext, updatePublishSettings, registerOnContextChange, }: RenderSettingsUiRequest) => {
    const intl = useIntl();
    ...
    return (<div className={styles.scrollContainer}>
      <Rows spacing="2u">
        {settingsUiContext?.outputType.displayName && (<Text>{settingsUiContext?.outputType.displayName}</Text>)}
        <FormField label={intl.formatMessage({
            defaultMessage: "Caption",
            description: "Label for the caption input field",
        })} control={(props) => (<TextInput {...props} value={settings.caption} onChange={(caption) => {
                setAndPropagateSettings({ ...settings, caption });
            }}/>)}/>
      </Rows>
    </div>);
};
const validatePublishRef = (publishRef: PublishSettings): PublishRefValidityState => {
