import { Avatar, Box, ImageCard, Placeholder, Rows, Text, TextPlaceholder, } from "@canva/app-ui-kit";
...
export const PostPreview = ({ previewMedia, settings, username, }: PreviewProps) => {
...
const UserInfo = ({ isLoading, username, }: {
    isLoading: boolean;
    username: string;
}) => {
...
const PreviewRenderer = ({ preview }: {
    ...
    if (preview.status === "error") {
        return (<ImageStatusText text={intl.formatMessage({
                ...
                description: "Text displayed when there is an error loading the preview",
            })}/>);
    }
    if (isImagePreviewReady(preview)) {
        return (<ImageCard alt={intl.formatMessage({
            ...
            }, { previewId: preview.id })} thumbnailUrl={preview.url}/>);
    }
    return (<Box width="full" height="full" padding="2u" display="flex" alignItems="center" justifyContent="center">
      <Text size="medium" tone="tertiary" alignment="center">
        <FormattedMessage defaultMessage="Preview not available" description="Text displayed when the preview type is not supported"/>
      </Text>
    </Box>);
};
