...
export const ImageGrid = () => {
    ...
    const onDragStart = async (event: DragEvent<HTMLElement>, image: ImageType) => {
    ...
    return (<Rows spacing="1u">
      <Text size="medium" variant="bold">
        <FormattedMessage defaultMessage="Select or drag to add to design" description="Instruction to the user on how they can add the generated image to their design"/>
      </Text>
      <Grid columns={2} spacing="2u">
        {generatedImages.map((image, index) => (<ImageCard key={index} thumbnailUrl={image.thumbnail.url} onClick={() => onImageClick(image)} ariaLabel={intl.formatMessage({
                defaultMessage: "Add image to design",
                description: "Aria label for the image card. When the image card is pressed, it will add the image to the design",
            })} alt={image.label} thumbnailHeight={THUMBNAIL_HEIGHT} borderRadius="standard" onDragStart={(event: DragEvent<HTMLElement>) => onDragStart(event, image)}/>))}
      </Grid>
    </Rows>);
};
